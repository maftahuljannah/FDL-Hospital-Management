    ...
    <div class="departments-section section-padding">
        <div class="container">
            ...
            <div class="departments-slider lazy">
                ...
                @foreach($departments->where('featured', true) as $department)
...
<script>
    ...
    $('#department').change(function(){
       ...
       $.ajax({
            url: `{{ route('doctors.ajax') }}`,
            method: `GET`,
            data:{
                dept_id: departmentId
            },
            success: function(res){
                let optionArr = [`<option disabled selected>Select a Doctor</option>`];

                res.forEach(doctor =>{
                    let optionHTML = `<option value="${doctor.id}">${doctor.name}</option>`
                    optionArr.push(optionHTML)
                })
                $('#doctor').html(optionArr)
            },
            error: function(err){
                console.log(err)
            }
    ...
    $('#doctor').change(function(){
        ...
        $.ajax({
            ...
            success: function(res){
               $('#date').html(res.availability_date)
               $('#time').html(res.availability_time)
            },
            error: function(err){
                console.log(err)
            }
